...
export type MarkdownRuntimeModules = {
...
export type RemarkPluginList = NonNullable<ReactMarkdownOptions['remarkPlugins']>;
export type RehypePluginList = NonNullable<ReactMarkdownOptions['rehypePlugins']>;
...
let cachedMarkdownRuntimeModules: MarkdownRuntimeModules | null = null;
let markdownRuntimeModulesPromise: Promise<MarkdownRuntimeModules> | null = null;
...
async function loadMarkdownRuntimeModules(): Promise<MarkdownRuntimeModules> {
    if (cachedMarkdownRuntimeModules) {
        return cachedMarkdownRuntimeModules;
    }

    if (!markdownRuntimeModulesPromise) {
        markdownRuntimeModulesPromise = Promise.all([
            ...
            cachedMarkdownRuntimeModules = nextRuntimeModules;
            return nextRuntimeModules;
        }).catch((error: unknown) => {
            markdownRuntimeModulesPromise = null;
            throw error;
        });
    }

    return markdownRuntimeModulesPromise;
}

export function normalizeMarkdownForRender(value: string): string {
    return repairDanglingHtmlTables(normalizeEscapedTableHtml(value));
}

export function useMarkdownRuntime(): MarkdownRuntimeState {
    ...
    const components = useMemo<MarkdownComponents>(() => ({
        table(props) {
            ...
            return createElement(
                ...
                createElement('table', tableProps)
            );
        },
    }), []);
